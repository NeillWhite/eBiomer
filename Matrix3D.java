...
class Matrix3D {
	...
	Matrix3D ( float ixx, float ixy, float ixz, float ixo,
			float iyx, float iyy, float iyz, float iyo,
			float izx, float izy, float izz, float izo ) {
		xx = ixx;
		xy = ixy;
		xz = ixz;
		xo = ixo;
	...
	void spacefillTransform(float v[], int tv[], int nvert, double factor) {
		float lxx = xx, lxy = xy, lxz = xz, lxo = xo;
		float lyx = yx, lyy = yy, lyz = yz, lyo = yo;
		float lzx = zx, lzy = zy, lzz = zz, lzo = zo;
		for (int i = nvert * 3; (i -= 3) >= 0;) {
			...
			tv[i    ] = (int) (x * lxx + y * lxy + z * lxz + lxo);
			tv[i + 1] = (int) (x * lyx + y * lyy + z * lyz + lyo);
			tv[i + 2] = (int) (factor*(x * lzx + y * lzy + z * lzz + lzo));
		}
	}

	void inverseTransform(float v[], int tv[], int nvert) {
		float lxx = xx, lxy = xy, lxz = xz, lxo = xo;
		float lyx = yx, lyy = yy, lyz = yz, lyo = yo;
		float lzx = zx, lzy = zy, lzz = zz, lzo = zo;
		float d = lxx*lyy*lzz-lxx*lyz*lzy-lyx*lxy*lzz+lyx*lxz*lzy+lzx*lxy*lyz-lzx*lxz*lyy;
		for (int i = nvert * 3; (i -= 3) >= 0;) {
			if ( d != 0 ){
				float x = tv[i] - lxo;
				float y = tv[i + 1] -lyo;
				float z = tv[i + 2] -lzo;
				v[i] = (x*(lyy*lzz-lyz*lzy)-y*(lxy*lzz-lxz*lzy)+z*(lxy*lyz-lxz*lyy))/d;
				v[i+1] = (-x*(lyx*lzz-lyz*lzx)+y*(lxx*lzz-lxz*lzx)-z*(lxx*lyz-lxz*lyx))/d;
	...
	void transform(float v[], int nvert) {
		float lxx = xx, lxy = xy, lxz = xz, lxo = xo;
		float lyx = yx, lyy = yy, lyz = yz, lyo = yo;
		float lzx = zx, lzy = zy, lzz = zz, lzo = zo;
		for (int i = nvert * 3; (i -= 3) >= 0;) {
			...
			float y = v[i + 1];
			float z = v[i + 2];
			v[i    ] = x * lxx + y * lxy + z * lxz + lxo;
			v[i + 1] = x * lyx + y * lyy + z * lyz + lyo;
			v[i + 2] = x * lzx + y * lzy + z * lzz + lzo;
		}
	}
	void inverseTransform(float v[], int nvert) {
		float lxx = xx, lxy = xy, lxz = xz, lxo = xo;
		float lyx = yx, lyy = yy, lyz = yz, lyo = yo;
		float lzx = zx, lzy = zy, lzz = zz, lzo = zo;
		float d = lxx*lyy*lzz-lxx*lyz*lzy-lyx*lxy*lzz+lyx*lxz*lzy+lzx*lxy*lyz-lzx*lxz*lyy;
		for (int i = nvert * 3; (i -= 3) >= 0;) {
			if ( d != 0 ){
				...
				float y = v[i + 1] -lyo;
				float z = v[i + 2] -lzo;
				v[i] = (x*(lyy*lzz-lyx*lzy)-y*(lxy*lzz-lxz*lzy)+z*(lxy*lyz-lxz*lyy))/d;
